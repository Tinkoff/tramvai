...
describe('test/integration/app/runFakeApp', () => {
  ...
  it('should run app', async () => {
    ...
    expect(application).toMatchInlineSnapshot(
      `"<div class=\\"layout__layout_28L1F\\"><div><h1>Tramvai<span role=\\"img\\" aria-label=\\"dummy icon\\">🥳</span></h1></div><div>Main Page <button>click link</button></div><div class=\\"Footer__footer_1hU3C\\"><div>this Footer in bootstrap</div></div></div>"`
    );
  });
